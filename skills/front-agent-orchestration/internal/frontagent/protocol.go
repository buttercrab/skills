...
func parseWorkAuthority(body string) (workAuthority, error) {
	...
	if result.SchemaVersion, err = scalarString(get("schema_version"), "schema_version"); err != nil || result.SchemaVersion != "work_authority/v1" {
		return workAuthority{}, fmt.Errorf("work authority schema_version must be work_authority/v1")
	}
	...
	packet, err := parsePacketAuthority(packetNode)
	if err != nil {
		return workAuthority{}, err
	}
...
func parsePacketAuthority(node *yaml.Node) (packetAuthority, error) {
	fields := []string{"packet_id", "task_id", "packet_path", "packet_revision", "protected_digest", "approval_id", "authority_classes", "coordinator_id", "coordinator_epoch", "state_generation", "lifecycle_status", "execution_head"}
