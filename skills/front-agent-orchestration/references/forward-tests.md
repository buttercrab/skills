...
1. Main startup: ask an agent to use `$front-agent-orchestration main`. It should run `main`, give the tokenless gateway command, wait for pairing, then run one-shot `front-agent listen --identity <main-id>`.
...
9. Repeat loop: after pairing and after every handled message, main should run or explicitly restart one-shot `front-agent listen`; gateway should drain with `listen --timeout 0` at human-turn boundaries.
...
12. Full-body listen: `front-agent listen --timeout 0` should print full valid unread protocol message bodies, not only inbox summary lines.
...
23. Queued drain: queue multiple valid unread messages, then run one-shot `listen --timeout 0`; it should print all currently available valid messages before exiting.
