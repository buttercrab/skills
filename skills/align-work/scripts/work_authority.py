#!/usr/bin/env python3
"""Validate the closed work_authority/v1 seam and packet transfer receipts."""
...
def validate_authority_shape(authority: dict[str, Any]) -> None:
    ...
    if authority["schema_version"] != "work_authority/v1":
    ...
    if set(binding) != PACKET_FIELDS:
        fail("E_SCHEMA", f"packet binding fields {sorted(set(binding) ^ PACKET_FIELDS)}")
...
def binding_from_state(repository: Path, packet: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "authority_classes": sorted(approval["authority_classes"]),
