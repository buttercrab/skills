---
name: align-work
description: Use as the outer alignment and approval workflow for coding when the user invokes $align-work, a material decision changes outcome or authority, an existing Align packet resumes, or work is destructive, externally mutating, cross-session, or cross-agent. Default to lightweight alignment for local reversible work; inspect enough, ask at most one decision-changing clarification round, recommend once, execute on approval, and continue through scope-neutral reversible plan drift without reapproval. Use durable packet mode for existing packets, continuity, destructive or irreversible, production, security/privacy, costly, or external work. Align owns approval and packet state; front-agent-orchestration is the human gateway and always durable. Read-only audits route to audit-technical-work; domain skills own mechanics. Do not auto-trigger for ordinary clear bounded coding, simple answers, or low-risk mechanical edits unless explicitly invoked.
---
...
5. Implement and verify within the approved envelope. Default to continuing: a difference from the approved plan is not itself an approval event. Stop only when you can name the concrete new authority required because the outcome, authorized surfaces, external effects, destructive or irreversible actions, trust/security/privacy boundary, cost ceiling, required gates, or partial-work disposition would materially change.
...
- Ask only about decisions that require authority beyond the current approval envelope. User-delegated judgment closes non-authority questions. This includes internal architecture and plan details that preserve the approved outcome and boundaries.
- Interpret approval envelopes broadly enough to execute the approved outcome. Continue through scope-neutral file changes, internal refactors, implementation substitutions, step additions or reordering, retries, bounded dependency adjustments, stronger verification, and ordinary in-scope fixes when they remain local or otherwise authorized, reversible, and within the same trust and risk boundary. If uncertain, prefer continuing unless you can state the concrete additional authority needed.
- Exact destructive allowlists, named namespaces, publication or deployment targets, and other sealed external scopes remain literal. A live target outside them requires new approval.
...
- `forbidden_actions`: ["delegate packet mutation to Front, Handoff, Goal Loop, or domain skills","infer external authority","create a durable packet solely because Align was explicitly invoked when no durable trigger applies","repeat lightweight clarification without a named unresolved decision that changes the approval envelope","request reapproval for scope-neutral reversible plan drift","downgrade explicit Align or matching packet state"]
